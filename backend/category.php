					...
					<div class="row">
						<div class="col-md-12">
							...
							<?php else : ?>
								...
								$statement = $pdo->prepare("SELECT * FROM news WHERE category_id=? ORDER BY news_id DESC");
								$statement->execute(array($category_id));
								...
								$statement = $pdo->prepare("SELECT * FROM news WHERE category_id=? LIMIT $start, $limit");
								$statement->execute(array($category_id));
								$result = $statement->fetchAll(PDO::FETCH_ASSOC);
