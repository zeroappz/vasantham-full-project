...
if(isset($_POST['form1'])) {
    ...
    if($valid == 1) {
    	...
    	if($path == '') {
			...
			$statement = $pdo->prepare("UPDATE photo SET photo_caption=?, p_category_id=? WHERE photo_id=?");
        	...
        	// updating into the database
			$statement = $pdo->prepare("UPDATE photo SET photo_caption=?, photo_name=?, p_category_id=? WHERE photo_id=?");
...
if(!isset($_REQUEST['id'])) {
	...
	$statement = $pdo->prepare("SELECT * FROM photo WHERE photo_id=?");
						...
						<div class="form-group">
							...
							<div class="col-sm-4">
								<select class="form-control" name="p_category_id">
									<?php
									$statement = $pdo->prepare("SELECT * FROM category_photo ORDER BY p_category_name ASC");
									$statement->execute();
									$result = $statement->fetchAll(PDO::FETCH_ASSOC);							
									foreach ($result as $row) {
