...
if(isset($_POST['form1'])) {
    ...
    if($path!='') {
        ...
        if( $ext!='jpg' && $ext!='png' && $ext!='jpeg' && $ext!='gif' ) {
            $valid = 0;
            $error_message .= 'You must have to upload jpg, jpeg, gif or png file for doctor photo<br>';
        }
    }
	...
	if($valid == 1) {
		...
		$statement = $pdo->prepare("SELECT * FROM doctor WHERE slug=? AND name!=?");
		$statement->execute(array($slug,$_POST['current_doctor_name']));
		...
		if($path == '' && $path1 == '') {
			$statement = $pdo->prepare("UPDATE doctor SET name=?, slug=?, designation_id=?, degree=?, detail=?, facebook=?, twitter=?, linkedin=?, youtube=?, google_plus=?, instagram=?, flickr=?, address=?, practice_location=?, phone=?, email=?, website=?, status=?, meta_title=?, meta_keyword=?, meta_description=? WHERE id=?");
		...
		if($path != '' && $path1 == '') {
			...
			$statement = $pdo->prepare("UPDATE doctor SET name=?, slug=?, designation_id=?, photo=?, degree=?, detail=?, facebook=?, twitter=?, linkedin=?, youtube=?, google_plus=?, instagram=?, flickr=?, address=?, practice_location=?, phone=?, email=?, website=?, status=?, meta_title=?, meta_keyword=?, meta_description=? WHERE id=?");
		...
		if($path == '' && $path1 != '') {
			...
			$final_name1 = 'doctor-banner-'.$_REQUEST['id'].'.'.$ext1;
        	move_uploaded_file( $path_tmp1, '../assets/uploads/'.$final_name1 );

			$statement = $pdo->prepare("UPDATE doctor SET name=?, slug=?, designation_id=?, banner=?, degree=?, detail=?, facebook=?, twitter=?, linkedin=?, youtube=?, google_plus=?, instagram=?, flickr=?, address=?, practice_location=?, phone=?, email=?, website=?, status=?, meta_title=?, meta_keyword=?, meta_description=? WHERE id=?");
		...
		if($path != '' && $path1 != '') {
			...
			$statement = $pdo->prepare("UPDATE doctor SET name=?, slug=?, designation_id=?, photo=?, banner=?, degree=?, detail=?, facebook=?, twitter=?, linkedin=?, youtube=?, google_plus=?, instagram=?, flickr=?, address=?, practice_location=?, phone=?, email=?, website=?, status=?, meta_title=?, meta_keyword=?, meta_description=? WHERE id=?");
    		$statement->execute(array($_POST['name'],$slug,$_POST['designation_id'],$final_name,$final_name1,$_POST['degree'],$_POST['detail'],$_POST['facebook'],$_POST['twitter'],$_POST['linkedin'],$_POST['youtube'],$_POST['google_plus'],$_POST['instagram'],$_POST['flickr'],$_POST['address'],$_POST['practice_location'],$_POST['phone'],$_POST['email'],$_POST['website'],$_POST['status'],$_POST['meta_title'],$_POST['meta_keyword'],$_POST['meta_description'],$_REQUEST['id']));
		}

	    $success_message = 'Doctor is updated successfully!';
	}
}
?>
...
if(!isset($_REQUEST['id'])) {
	...
	$statement = $pdo->prepare("SELECT * FROM doctor WHERE id=?");
...
<section class="content-header">
	...
	<div class="content-header-right">
		<a href="doctor.php" class="btn btn-primary btn-sm">View All</a>
	</div>
</section>
	...
	<div class="row">
		<div class="col-md-12">
			...
			<form class="form-horizontal" action="" method="post" enctype="multipart/form-data">
				...
				<input type="hidden" name="current_doctor_name" value="<?php echo $name; ?>">
					...
					<div class="box-body">
						...
						<div class="form-group">
				            ...
				            <div class="col-sm-3">
								...
								<?php
				            	$i=0;
				            	$statement = $pdo->prepare("SELECT * FROM designation ORDER BY designation_name ASC");
						...
						<div class="form-group">
							...
							<div class="col-sm-9" style="padding-top:5px">
								<img src="<?php echo BASE_URL; ?>assets/uploads/<?php echo $photo; ?>" alt="Doctor Photo" style="width:200px;">
							</div>
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							...
							<div class="col-sm-9" style="padding-top:5px">
								<img src="<?php echo BASE_URL; ?>assets/uploads/<?php echo $banner; ?>" alt="Doctor Banner" style="width:200px;">
							</div>
						</div>
						<div class="form-group">
