...
if(isset($_POST['form1'])) {
    ...
    if(empty($_POST['faq_category_name'])) {
    	...
    	$statement = $pdo->prepare("SELECT * FROM faq_category WHERE faq_category_id=?");
		...
		$statement = $pdo->prepare("SELECT * FROM faq_category WHERE faq_category_name=? and faq_category_name!=?");
    ...
    if($valid == 1) {
		...
		$statement = $pdo->prepare("UPDATE faq_category SET faq_category_name=? WHERE faq_category_id=?");
