...
if(isset($_POST['form1'])) {
    ...
    if($path!='') {
    	...
    	$valid = 0;
        $error_message .= 'You must have to select a photo for doctor photo<br>';
    }
	...
	if($valid == 1) {
		...
		$statement = $pdo->prepare("SHOW TABLE STATUS LIKE 'doctor'");
		$statement->execute();
		...
		$statement = $pdo->prepare("INSERT INTO doctor (name,slug,designation_id,photo,banner,degree,detail,facebook,twitter,linkedin,youtube,google_plus,instagram,flickr,address,practice_location,phone,email,website,status,meta_title,meta_keyword,meta_description) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
