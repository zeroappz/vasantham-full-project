	...
	if($_SESSION['user']['role'] == 'Super Admin') {
	    ...
	    if(empty($_POST['email'])) {
	    	...
	    	if (filter_var($_POST['email'], FILTER_VALIDATE_EMAIL) === false) {
		    	...
		    	$statement = $pdo->prepare("SELECT * FROM user WHERE email=? and email!=?");
	    ...
	    if($valid == 1) {
			...
			$statement = $pdo->prepare("UPDATE user SET full_name=?, email=?, phone=? WHERE id=?");
		...
		// updating the database
		$statement = $pdo->prepare("UPDATE user SET phone=? WHERE id=?");
