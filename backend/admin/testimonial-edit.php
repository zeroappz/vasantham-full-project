...
if(isset($_POST['form1'])) {
	...
	if($valid == 1) {
		...
		if($path == '') {
			$statement = $pdo->prepare("UPDATE testimonial SET name=?, designation=?, company=?, comment=? WHERE id=?");
        	...
        	$statement = $pdo->prepare("UPDATE testimonial SET name=?, designation=?, company=?, photo=?, comment=? WHERE id=?");
...
if(!isset($_REQUEST['id'])) {
	...
	$statement = $pdo->prepare("SELECT * FROM testimonial WHERE id=?");
