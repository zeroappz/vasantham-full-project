...
if(isset($_POST['form1'])) {
	...
	if(empty($_POST['news_title'])) {
    	...
    	$statement = $pdo->prepare("SELECT * FROM news WHERE news_id=?");
	...
	if($valid == 1) {
	    ...
	    if($previous_photo == '' && $path == '') {
	    	$statement = $pdo->prepare("UPDATE news SET news_title=?, news_slug=?, news_content=?, news_date=?, category_id=?, publisher=?, meta_title=?, meta_keyword=?, meta_description=? WHERE news_id=?");
	    ...
	    if($previous_photo == '' && $path != '') {
	    	...
	    	$statement = $pdo->prepare("UPDATE news SET news_title=?, news_slug=?, news_content=?, news_date=?, photo=?, category_id=?, publisher=?, meta_title=?, meta_keyword=?, meta_description=? WHERE news_id=?");
