...
if(isset($_POST['form1'])) {
    ...
    if($valid == 1) {
    	if($path=='') {
			...
			$statement = $pdo->prepare("UPDATE advertisement_home SET adv_url=?, adv_status=? WHERE adv_id=1");
			$statement->execute(array($_POST['adv_url'],$_POST['adv_status']));
    	} else {
    		unlink('../assets/uploads/'.$_POST['previous_photo']);

			$final_name = 'adv-1'.'.'.$ext;
			move_uploaded_file( $path_tmp, '../assets/uploads/'.$final_name );
			
			// updating the database
			$statement = $pdo->prepare("UPDATE advertisement_home SET adv_photo=?, adv_url=?, adv_status=? WHERE adv_id=1");
			$statement->execute(array($final_name,$_POST['adv_url'],$_POST['adv_status']));
    	}
    	$success_message = 'Advertisement (Inside Header) is updated successfully.';
    }
}
...
if(isset($_POST['form2'])) {
    ...
    if($valid == 1) {
    	if($path=='') {
    		// updating the database
			$statement = $pdo->prepare("UPDATE advertisement_home SET adv_url=?, adv_status=? WHERE adv_id=2");
			$statement->execute(array($_POST['adv_url'],$_POST['adv_status']));
    	} else {
    		unlink('../assets/uploads/'.$_POST['previous_photo']);

			$final_name = 'adv-2'.'.'.$ext;
			move_uploaded_file( $path_tmp, '../assets/uploads/'.$final_name );
			
			// updating the database
			$statement = $pdo->prepare("UPDATE advertisement_home SET adv_photo=?, adv_url=?, adv_status=? WHERE adv_id=2");
			$statement->execute(array($final_name,$_POST['adv_url'],$_POST['adv_status']));
    	}
    	$success_message = 'Advertisement (Under Featured News) is updated successfully.';
    }
}
?>
...
<section class="content-header">
	<div class="content-header-left">
		<h1>Advertisement (Home)</h1>
	</div>
</section>


<?php
$statement = $pdo->prepare("SELECT * FROM advertisement_home");
$statement->execute();
		...
		<div class="col-md-12">
				...
				<div class="nav-tabs-custom">
					<ul class="nav nav-tabs">
						<li class="active"><a href="#tab_1" data-toggle="tab">Inside Header</a></li>
						<li><a href="#tab_2" data-toggle="tab">Under Featured News</a></li>						
					</ul>
