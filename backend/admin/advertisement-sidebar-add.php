...
if(isset($_POST['form1'])) {
    ...
    if($valid == 1) {
		...
		$statement = $pdo->prepare("SHOW TABLE STATUS LIKE 'advertisement_sidebar'");
		$statement->execute();
		...
		$final_name = 'adv-sidebar-'.$ai_id.'.'.$ext;
        move_uploaded_file( $path_tmp, '../assets/uploads/'.$final_name );

		// saving into the database
		$statement = $pdo->prepare("INSERT INTO advertisement_sidebar (adv_location,adv_photo,adv_url) VALUES (?,?,?)");
		$statement->execute(array($_POST['adv_location'],$final_name,$_POST['adv_url']));

    	$success_message = 'Advertisement (Sidebar) is added successfully.';
    }
}
?>
...
<section class="content-header">
	<div class="content-header-left">
		<h1>Add Advertisement (Sidebar)</h1>
	</div>
