...
if(isset($_POST['form1'])) {
    ...
    if(empty($_POST['designation_name'])) {
		...
		$statement = $pdo->prepare("SELECT * FROM designation WHERE designation_name=? and designation_name!=?");
    ...
    if($valid == 1) {
		...
		$statement = $pdo->prepare("UPDATE designation SET designation_name=? WHERE designation_id=?");
		$statement->execute(array($_POST['designation_name'],$_REQUEST['id']));

    	$success_message = 'Designation is updated successfully.';
    }
}
?>
