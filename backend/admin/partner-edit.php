...
if(isset($_POST['form1'])) {
	...
	if($valid == 1) {
		...
		if($path == '') {
			$statement = $pdo->prepare("UPDATE partner SET name=?, url=? WHERE id=?");
        	...
        	$statement = $pdo->prepare("UPDATE partner SET name=?, url=?, photo=? WHERE id=?");
...
if(!isset($_REQUEST['id'])) {
	...
	$statement = $pdo->prepare("SELECT * FROM partner WHERE id=?");
