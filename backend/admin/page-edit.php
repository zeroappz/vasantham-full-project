...
if(isset($_POST['form1'])) {
    ...
    if(empty($_POST['page_name'])) {
    	...
    	$statement = $pdo->prepare("SELECT * FROM page WHERE page_id=?");
    ...
    if($valid == 1) {
		...
		if($total) {
   		...
   		if($path == '') {
			...
			$statement = $pdo->prepare("UPDATE page SET page_name=?, page_slug=?, page_content=?,page_layout=?, status=?, meta_title=?, meta_keyword=?, meta_description=? WHERE page_id=?");
        	...
        	move_uploaded_file( $path_tmp, '../assets/uploads/'.$final_name );
   			...
   			// updating into the database
			$statement = $pdo->prepare("UPDATE page SET page_name=?, page_slug=?, page_content=?,page_layout=?, banner=?, status=?, meta_title=?, meta_keyword=?, meta_description=? WHERE page_id=?");
