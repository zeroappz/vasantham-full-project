...
if(isset($_POST['form1'])) {
	...
	if($valid == 1) {
		...
		if($path == '') {
			$statement = $pdo->prepare("UPDATE slider SET heading=?, subheading=?, content=?, button_text=?, button_url=?, position=?, status=? WHERE id=?");
        	...
        	$statement = $pdo->prepare("UPDATE slider SET photo=?, heading=?, subheading=?, content=?, button_text=?, button_url=?, position=?, status=? WHERE id=?");
