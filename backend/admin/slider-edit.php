...
if (isset($_POST['form1'])) {
	...
	if ($valid == 1) {
		...
		if ($path == '') {
			$statement = $pdo->prepare("UPDATE slider SET position=?, status=? WHERE id=?");
			$statement->execute(array($_POST['position'], $_POST['status'], $_REQUEST['id']));
		} else {
			...
			$statement = $pdo->prepare("UPDATE slider SET photo=?, position=?, status=? WHERE id=?");
			$statement->execute(array($final_name, $_POST['position'], $_POST['status'], $_REQUEST['id']));
		}
...
if (!isset($_REQUEST['id'])) {
	...
	$statement = $pdo->prepare("SELECT * FROM slider WHERE id=?");
